...
CREATE VIEW data.pitching_stats_player_lifetime AS
 SELECT a.player_name,
   ...
   FROM (data.pitching_stats_all_appearances p
     JOIN data.players_extended_current a ON (((a.player_id)::text = (p.player_id)::text)))
...
CREATE VIEW data.pitching_stats_player_season AS
 SELECT a.player_name,
   ...
   FROM (data.pitching_stats_all_appearances p
     JOIN data.players_extended_current a ON (((a.player_id)::text = (p.player_id)::text)))
  WHERE (NOT p.is_postseason)
  GROUP BY a.player_name, p.player_id, p.team_id, p.season;
           ...
           FROM ( SELECT DISTINCT t_1.team_id,
                ...
                UNION
                 SELECT DISTINCT dt_1.team_id,
                   ...
                   FROM taxa.division_teams dt_1) x) ts
     JOIN data.teams t ON ((((ts.team_id)::text = (t.team_id)::text) AND (t.valid_until IS NULL) AND (t.valid_from <= (ts.timestampd + '00:00:00.001'::interval)) AND (ts.timestampd < COALESCE((t.valid_until)::timestamp with time zone, (now() + '00:00:00.001'::interval))))))
...
CREATE MATERIALIZED VIEW data.players_info_expanded_all AS
 SELECT p.player_id,
   ...
   FROM ((((((data.players p
     JOIN data.player_status_flags ps ON (((p.player_id)::text = (ps.player_id)::text)))
     ...
     JOIN data.teams_info_expanded_all t ON ((((r.team_id)::text = (t.team_id)::text) AND (t.valid_until IS NULL))))
     LEFT JOIN taxa.blood xb ON ((p.blood = xb.blood_id)))
     LEFT JOIN taxa.coffee xc ON ((p.coffee = xc.coffee_id)))
...
SELECT pg_catalog.setval('data.chronicler_hash_game_event_chronicler_hash_game_event_id_seq', 8311281, true);
...
SELECT pg_catalog.setval('data.game_event_base_runners_id_seq', 10287884, true);
...
SELECT pg_catalog.setval('data.player_events_id_seq', 67223, true);
...
SELECT pg_catalog.setval('data.team_modifications_team_modifications_id_seq', 1057, true);
...
SELECT pg_catalog.setval('data.time_map_time_map_id_seq', 11931624, true);
