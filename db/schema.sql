...
CREATE VIEW data.pitching_stats_player_season AS
 SELECT l.player_name,
    ...
    sum(p.hits_allowed) AS hits_allowed
   FROM (data.pitching_stats_all_appearances p
     JOIN data.players_extended_current l ON (((p.player_id)::text = (l.player_id)::text)))
  WHERE (NOT p.is_postseason)
  GROUP BY p.player_id, p.season, l.player_name;
...
SELECT pg_catalog.setval('data.team_positions_team_position_id_seq', 17180, true);
