...
BEGIN
	...
	SET url_slug = replace(regexp_replace(lower(unaccent(player_name)), '[^A-Za-z'' ]', '','g'),' ','-')
	WHERE url_slug IS NULL
	AND NOT exists
	(
		SELECT 1 
		FROM DATA.players x
		WHERE x.url_slug = replace(regexp_replace(lower(unaccent(x.player_name)), '[^A-Za-z'' ]', '','g'),' ','-')
...
BEGIN
	UPDATE data.teams p
	SET url_slug = replace(regexp_replace(lower(unaccent(nickname)), '[^A-Za-z'' ]', '','g'),' ','-')
	WHERE url_slug IS NULL
	AND NOT exists
	(
		SELECT 1 
		FROM DATA.teams x
		WHERE x.url_slug = replace(regexp_replace(lower(unaccent(x.nickname)), '[^A-Za-z'' ]', '','g'),' ','-')
...
CREATE VIEW data.batting_stats_player_playoffs_season AS
 SELECT p.player_name,
   ...
   FROM (data.batting_stats_all_events a
     JOIN data.players_extended_current p ON (((a.player_id)::text = (p.player_id)::text)))
  WHERE a.is_postseason
  GROUP BY a.player_id, p.player_name, a.batter_team_id, a.season;
...
CREATE VIEW data.pitching_records_player_single_game AS
 SELECT a.rank,
  ...
  WHERE (a.rank = 1)
  ORDER BY a.stat, a.season, a.day, a.game_id;
...
SELECT pg_catalog.setval('data.game_event_base_runners_id_seq', 4817560, true);
...
SELECT pg_catalog.setval('data.player_events_id_seq', 4784, true);
...
SELECT pg_catalog.setval('data.player_modifications_player_modifications_id_seq', 1644, true);
...
SELECT pg_catalog.setval('data.players_id_seq', 56181, true);
...
SELECT pg_catalog.setval('data.team_modifications_team_modifications_id_seq', 116, true);
...
SELECT pg_catalog.setval('data.team_positions_team_position_id_seq', 12348, true);
...
SELECT pg_catalog.setval('data.teams_id_seq', 411, true);
...
SELECT pg_catalog.setval('data.time_map_time_map_id_seq', 5581521, true);
...
SELECT pg_catalog.setval('taxa.player_url_slugs_player_url_slug_id_seq', 3802, true);
...
CREATE TRIGGER player_insert AFTER INSERT ON data.players FOR EACH STATEMENT EXECUTE FUNCTION data.player_slug_creation();
...
CREATE TRIGGER team_insert AFTER INSERT ON data.teams FOR EACH STATEMENT EXECUTE FUNCTION data.team_slug_creation();
